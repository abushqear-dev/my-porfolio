export const personalInfo = {
  name: "Mohammed Abu-Shqear",
  title: "Full-Stack Developer",
  bio: "Junior Full-Stack Developer specializing in Next.js and modern React ecosystems. Built a prototype educational game using Figma and Unity. Strong focus on UI/UX, clean architecture, and scalable frontend-backend integration. Actively seeking a junior role where I can grow in real-world product development.",
  phone: "+962 78 974 5350",
  email: "m.abushqear.dev@gmail.com",
  location: "Amman, Jordan",
  links: {
    linkedin: "https://www.linkedin.com/in/abushqear-dev/",
    github: "https://github.com/abushqear-dev"
  },
  version: "1.0.0"
};

export const projects = [
  {
    title: "CodeQuest",
    link: "https://github.com/abushqear-dev/CodeQuest",
    period: "2024 February - 2024 June",
    description: "An interactive game that teaches programming concepts and domains through engaging gameplay. The game offers a practical learning environment, enhancing users understanding of programming fundamentals while fostering problem-solving skills.",
    points: [
      "Led the UI/UX design phase.",
      "Created and designed promotional materials, including the official game poster.",
      "Played a key role in the project management.",
      "Designed a whole level."
    ]
  }
];

export const experience = [
  {
    title: "Freelance Translator",
    company: "Trek Medics International",
    period: "2023 June - 2025 January",
    points: [
      "Translated Beacon platform content into Arabic, ensuring accurate and culturally appropriate translations.",
      "Conducted thorough testing to ensure the translations were accurate and culturally appropriate."
    ]
  }
];

export const skills = {
  frontend: "HTML & CSS, TypeScript, JavaScript, React.js, Next.js, Tailwind CSS, TanStack Query.",
  backend: "Node.js, Express.js, PostgreSQL, Prisma.",
  tools: "Figma, VS Code, Postman, Git & GitHub.",
  soft: "Collaboration & Communication, Fast Learning & Adaptability, Problem Solving, Time Management."
};
